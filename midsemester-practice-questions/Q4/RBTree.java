...
public class RBTree<T extends Comparable<T>> {
	...
	public boolean testProp3() {
		// START YOUR CODE
		if (root == null) {
			return true; // Empty tree satisfies property 3
		}

		List<Node<T>> leafNodes = root.childrenLeaves();
		int expectedBlackCount = leafNodes.get(0).blackParentCount();
		for (Node<T> leafNode : leafNodes) {
			if (leafNode.blackParentCount() != expectedBlackCount) {
				return false;
			}
		}
